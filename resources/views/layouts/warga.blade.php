...
<body class="font-body-md antialiased bg-white md:bg-surface-dim min-h-screen text-on-surface"
    x-data="{
...
>
    ...
    <div class="flex h-screen overflow-hidden relative">
        ...
        <!-- DESKTOP SIDEBAR -->
        <aside class="hidden md:flex flex-col w-72 bg-white border-r border-outline h-screen shrink-0 relative z-40 shadow-sm">
            <!-- Brand/Header -->
            ...
            <nav class="flex-1 overflow-y-auto py-6 px-4 space-y-1.5">
                <p class="text-[10px] font-bold text-on-surface-variant uppercase tracking-widest px-4 mb-3">Menu Utama</p>
                <a href="{{ route('warga.dashboard') }}" class="flex items-center gap-3 px-4 py-3 rounded-xl transition-all {{ request()->routeIs('warga.dashboard') ? 'bg-primary text-white shadow-md shadow-primary/20' : 'text-on-surface-variant hover:bg-surface hover:text-on-surface' }}">
                ...
                <a href="{{ route('warga.aktivitas.index') }}" class="flex items-center gap-3 px-4 py-3 rounded-xl transition-all {{ request()->routeIs('warga.aktivitas.*') ? 'bg-primary text-white shadow-md shadow-primary/20' : 'text-on-surface-variant hover:bg-surface hover:text-on-surface' }}">
                ...
                <a href="{{ route('warga.edukasi.index') }}" class="flex items-center gap-3 px-4 py-3 rounded-xl transition-all {{ request()->routeIs('warga.edukasi.*') ? 'bg-primary text-white shadow-md shadow-primary/20' : 'text-on-surface-variant hover:bg-surface hover:text-on-surface' }}">
                ...
                <p class="text-[10px] font-bold text-on-surface-variant uppercase tracking-widest px-4 mb-3">Pengaturan</p>
                
                <a href="{{ route('warga.profil.index') }}" class="flex items-center gap-3 px-4 py-3 rounded-xl transition-all {{ request()->routeIs('warga.profil.*') ? 'bg-primary text-white shadow-md shadow-primary/20' : 'text-on-surface-variant hover:bg-surface hover:text-on-surface' }}">
            ...
            <div class="p-4 border-t border-outline">
                <button @click="showLogoutModal = true" class="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-red-600 hover:bg-red-50 transition-colors">
        ...
        <div class="flex-1 flex flex-col h-screen overflow-hidden relative bg-white md:bg-surface/50">
            ...
            <header class="md:hidden sticky top-0 z-40 bg-white/90 backdrop-blur-md border-b-2 border-outline px-4 pt-[calc(env(safe-area-inset-top,0px)+1.25rem)] pb-5 flex items-center justify-between shrink-0">
                @yield('header')
            </header>

            <!-- Desktop Header -->
            <header class="hidden md:flex h-20 bg-white border-b border-outline px-8 items-center justify-between shrink-0 shadow-sm sticky top-0 z-30">
                ...
                <div class="flex items-center gap-4">
                    <div class="relative" x-data="{ showNotif: false }" @click.away="showNotif = false">
                        ...
                        <div x-show="showNotif" x-transition class="absolute right-0 mt-2 w-80 bg-white rounded-2xl shadow-xl border border-outline overflow-hidden z-50" style="display:none;">
            ...
            <nav class="md:hidden fixed bottom-0 w-full bg-white border-t-2 border-outline pb-safe z-[60] rounded-t-2xl">
                <div class="flex justify-around items-center h-16 px-2">
                    <a href="{{ route('warga.dashboard') }}" class="flex flex-col items-center justify-center w-full h-full space-y-1 {{ request()->routeIs('warga.dashboard') ? 'text-primary' : 'text-on-surface-variant hover:text-on-surface' }}">
                    ...
                    <a href="{{ route('warga.aktivitas.index') }}" class="flex flex-col items-center justify-center w-full h-full space-y-1 {{ request()->routeIs('warga.aktivitas.*') ? 'text-primary' : 'text-on-surface-variant hover:text-on-surface' }}">
                    ...
                    <a href="{{ route('warga.edukasi.index') }}" class="flex flex-col items-center justify-center w-full h-full space-y-1 {{ request()->routeIs('warga.edukasi.*') ? 'text-primary' : 'text-on-surface-variant hover:text-on-surface' }}">
                    ...
                    <a href="{{ route('warga.profil.index') }}" class="flex flex-col items-center justify-center w-full h-full space-y-1 {{ request()->routeIs('warga.profil.*') ? 'text-primary' : 'text-on-surface-variant hover:text-on-surface' }}">
            ...
            <div class="fixed top-4 left-1/2 -translate-x-1/2 md:top-6 md:left-auto md:right-8 md:translate-x-0 z-[100] w-[90%] max-w-sm pointer-events-none pt-safe md:pt-0">
            ...
            <div x-show="showLogoutModal" x-transition.opacity class="fixed inset-0 bg-black/50 z-[200] backdrop-blur-sm flex items-center justify-center p-4" style="display:none;" @click.self="showLogoutModal = false">
                <div x-show="showLogoutModal" 
                    ...
                    <p class="text-sm text-on-surface-variant mb-8">Anda harus masuk kembali untuk memesan layanan atau melapor.</p>
                    <div class="flex gap-3">
                        <button @click="showLogoutModal = false" class="flex-1 font-bold text-on-surface-variant py-3 rounded-xl bg-surface border border-outline hover:bg-surface-variant transition-colors">Batal</button>
                        <a href="/" class="flex-1 font-bold text-white py-3 rounded-xl bg-red-600 hover:bg-red-700 shadow-lg shadow-red-600/30 transition-colors flex items-center justify-center">Keluar</a>
                    </div>
                </div>
            </div>
